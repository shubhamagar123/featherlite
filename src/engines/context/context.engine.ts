 ...
 *
 * It assembles the complete runtime context for a conversation and returns a
 * single `ConversationContextDTO`. This is the ONLY collaborator the (future)
 * Conversation Engine depends on: the Conversation Engine never talks to the
 * World Engine, Companion Engine, or services directly.
 */
...
export class ContextEngine implements IContextEngine {
  ...
  async assembleContext(request: ContextRequest): Promise<IResult<ConversationContextDTO>> {
    try {
      ...
      this.logger.error({ err: error }, 'Failed to assemble conversation context');
      return Result.failure(new Error('Failed to assemble conversation context'));
    }
  }
