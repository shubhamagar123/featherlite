...
import { ConversationEngine } from '@engines/conversation/conversation.engine';
...
/** Extraction engine whose return value can differ call-to-call. */
...
describe('ConversationEngine', () => {
  it('(a) appends a consent question when the Memory Extraction Engine flags a candidate', async () => {
    ...
    expect(result.value?.reply).toContain('Hi there!');
    expect(result.value?.reply).toContain(candidate.content);
    // Proposing a candidate must never, by itself, write anything.
    expect(persistSpy).not.toHaveBeenCalled();
  });

  it('(b) a "yes" reply on the next turn produces exactly one Memory Service write', async () => {
    const candidate = makeCandidate();
    const { service: memoryService, persistSpy } = makeFakeMemoryService();
    ...
    const engine = new ConversationEngine({
      ...
      // Turn 1 flags a candidate; turn 2 flags nothing new.
    ...
    expect(persistSpy).toHaveBeenCalledWith(candidate, {
      granted: true,
      sourceMessageId: candidate.sourceMessageId,
    });
  });

  it('(c) a "no" reply on the next turn produces zero Memory Service writes', async () => {
    const candidate = makeCandidate();
    const { service: memoryService, persistSpy } = makeFakeMemoryService();
  ...
  it('(c) no reply at all (candidate proposed, conversation never continues) produces zero writes', async () => {
    const candidate = makeCandidate();
    const { service: memoryService, persistSpy } = makeFakeMemoryService();
  ...
  it('an unrelated follow-up message on the next turn is treated as "not granted", not as a crash', async () => {
    const candidate = makeCandidate();
    const { service: memoryService, persistSpy } = makeFakeMemoryService();
    ...
    const turn2 = await engine.sendMessage({
      ...
      companionId: COMPANION_ID,
      messageId: 'msg-2',
      message: 'anyway, what is the weather like today?',
    });
  ...
  it('does not touch the Memory Service at all when no candidate is ever flagged', async () => {
    const { service: memoryService, persistSpy } = makeFakeMemoryService();
    ...
    const engine = new ConversationEngine({
      ...
      promptOrchestrator: makeFakePromptOrchestrator(),
      llmGateway: makeFakeLLMGateway(),
      memoryExtractionEngine: makeFakeExtractionEngine(null),
      memoryService,
    });
    ...
    expect(result.isSuccess).toBe(true);
    expect(result.value?.consentQuestionAsked).toBe(false);
    expect(result.value?.consentResolution).toBeUndefined();
    expect(persistSpy).not.toHaveBeenCalled();
  });
});
